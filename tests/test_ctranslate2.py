...
from pipeline.translate.translate_ctranslate2 import DecoderConfig, make_batches
...
def generate_random_lines() -> str:
    lines = []
    for i in range(20):
        lines.append(("* " * (i + 1)).strip())
    random.seed(9873243)
    lines.sort(key=lambda x: random.random())
    return "\n".join(lines)


class FakeEncoder:
    def Encode(self, line: str, out_type=None):
        return line.split(" ")
...
make_batches_cases = {
    # With a maxi-batch of 1000, every line is in the same "maxi" batch.
    "1000": [
        [
            "*",
            "* *",
    ...
    # With a maxi-batch of 14, the lines are split into 3 different "maxi" batches.
    "14": [
        [
            "*",
            "* *",
...
@pytest.mark.parametrize("cases", ["maxi-batch-1000", "max-batch-14"])
def test_make_batches(cases: str, data_dir: DataDir):
    """
    Unit test for make_batches, which is not directly testable from run_task.
    """
    lines_zst = Path(data_dir.create_zst("lines.zst", generate_random_lines()))
    original_lines = data_dir.read_text("lines.zst").split("\n")
    ...
    maxi_batch_sentences = cases.split("-").pop()
    decoder_config = DecoderConfig(
        ["--mini-batch-words", "30", "--maxi-batch", maxi_batch_sentences]
    )

    batches = []
    for mini_batch in make_batches(lines_zst, decoder_config, FakeEncoder()):
        batches.append([" ".join(batch) for batch in mini_batch])

    assert batches == make_batches_cases[maxi_batch_sentences]
    assert len(flatten(batches)) == len(original_lines)

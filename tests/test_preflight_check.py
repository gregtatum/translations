...
def test_task_training_config():
    output, opened_url = get_preflight_check_output("--only", "training_config")
    assert "Artifacts" not in output
    assert "Task Commands" not in output
    assert "Training config" in output
    assert "Visualization" not in output

    assert '"marian-args"' in output
    assert not opened_url

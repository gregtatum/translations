...
void loadItems(const void* current, std::vector<io::Item>& items, bool mapped) {
  ...
  for(int i = 0; i < numHeaders; ++i) {
    // if(items[i].mapped && !isIntgemm(items[i].type)) { // memory-mapped, hence only set pointer.
    // At the moment it intgemm matrices can't be used without processing
    //   items[i].ptr = get<char>(current, headers[i].dataLength);
    // } else { // reading into item data
    items[i].mapped
        = false;  // Completely disable MMAP support for any models. We do not use it in bergamot
                  // and we hijack this codepath for binary model loading. If this is not set, we
                  // trigger node_initializers.cpp:186. This one just assigns the memory ptr to the
                  // tensor if set to true, but at the moment We are preparing some things on demand
                  // (the bottom portion of this code). Once we stop doing that, we can use the full
                  // mmap codepath Also when using the full mmap codepath, we need to uncomment
                  // expression_graph.h:582

...
def main() -> None:
    ...
    for potential_model in potential_models:
        url = urljoin(url_prefix, potential_model)
    ...
    for potential_decoder in potential_decoders:
        url = urljoin(url_prefix, potential_decoder)
    ...
    if not src_vocab_url or not trg_vocab_url:
        shared_vocab_url = urljoin(url_prefix, "vocab.spm")
        if location_exists(shared_vocab_url):
            ...
            src_vocab_url = urljoin(url_prefix, f"vocab.{src_locale}.spm")
            trg_vocab_url = urljoin(url_prefix, f"vocab.{trg_locale}.spm")
        else:
            raise ValueError(

...
def trigger_training(decision_task_id: str, config: dict[str, any], dry_run: bool):
    ...
    if dry_run:
        ...
        print("\nTrain Action:", json.dumps(hook_payload, indent=2))
    else:
...
def validate_taskcluster_credentials():
    ...
    cmd = f'eval `TASKCLUSTER_ROOT_URL="{ROOT_URL}" taskcluster signin --scope "hooks:trigger-hook:project-translations/in-tree-action-1-train/*"`'

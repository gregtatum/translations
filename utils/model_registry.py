...
class Corpus:
    ...
    def from_mono_tasks(
        ...
        source_task = (
            ...
            find_latest_task(tasks, match_by_label(r"^collect-mono-trg-")) or
            find_latest_task(tasks, match_by_label(r"^collect-distillation-mono-"))
        )
        target_task = (
            # This task was renamed.
            find_latest_task(tasks, match_by_label(r"^collect-mono-src-")) or
            find_latest_task(tasks, match_by_label(r"^collect-backtranslations-"))
        )
...
def collect_corpora(training_run: TrainingRun, tasks: list[Task]):
    ...
    training_run.backtranslations_corpus_aligned = WordAlignedCorpus.from_task(
        ...
        (
            # This task was renamed.
            find_latest_task(tasks, match_by_label(r"^alignments-backtranslated-")) or
            find_latest_task(tasks, match_by_label(r"^alignments-backtranslations-"))
        ),
    )
    training_run.distillation_corpus_aligned = WordAlignedCorpus.from_task(
        ...
        (
            ...
            find_latest_task(tasks, match_by_label(r"^alignments-student-")) or
            find_latest_task(tasks, match_by_label(r"^alignments-distillation-"))
        ),
    )
    ...
    training_run.parallel_corpus = Corpus.from_task(
        ...
        (
            # This task was renamed.
            find_latest_task(tasks, match_by_label(r"^merge-corpus-")) or
            find_latest_task(tasks, match_by_label(r"^merge-cleaned-parallel-"))
        ),
    )

...
@dataclass(kw_only=True)
class CorporaUrls(KebabDataclass):
    src: str
    trg: str
    aln: str
...
@dataclass(kw_only=True)
class Corpora(KebabDataclass):
    backtranslations: CorporaUrls
    original_parallel: CorporaUrls
    student_distillation: CorporaUrls
...
class TrainingConfig(KebabDataclass):
    ...
    # Bypass back translations, or the teacher model by providing a pre-existing corpus.
    corpora: Optional[Corpora] = None

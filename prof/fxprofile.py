    ...
    return {
        "meta": {
            ...
            "categories": [
                ...
                {"name": "Python Package", "color": "purple", "subcategories": ["Other"]},
                {"name": "Site Package", "color": "orange", "subcategories": ["Other"]},
                {"name": "User Code", "color": "lightblue", "subcategories": ["Other"]},
            ],
            "product": "Python cProfile",
            "stackwalk": 0,
            "toolkit": "",
            "version": 29,
...
def build_profile():
    ...
    for prof_path in glob.glob('*.prof'):
        print("Processing", prof_path)
        if prof_path.endswith("combined.prof"):
            continue
        stats = pstats.Stats(prof_path)
        stats_dict: StatsDict = stats.stats
        roots = get_roots(stats_dict)

        # Compute the total time and callers dict for the root.
        total_time = 0.0
        callers_dict = {}
        for func_key in roots:
            func_info = stats_dict[func_key]
            total_time += func_info[3]
            callers_dict[func_key] = (func_info[0], func_info[1], func_info[2], func_info[3])

        # Create a new root with the script name
        root_key = (prof_path, 0, os.path.basename(prof_path))
        stats_dict[root_key] = (1, 1, 0.0, total_time, callers_dict)
        ...
        for func_key, func_info in stats_dict.items():
            ...
            path_string_index = get_string_index(path)
            try:
                resource_index = resource_table["name"].index(path_string_index)
            except ValueError:
                resource_index = resource_table["length"]
                resource_table["name"].append(path_string_index)
        ...
        visited_funcs: dict[FuncKey, True] = {}
        ...
        def recursive_add_frame_table(func_key: FuncKey, prefix: Optional[int] = None, depth = 0) -> float:
            ...
            frame_table["category"].append(category)
            ...
            stack_table["category"].append(category)
        ...
        recursive_add_frame_table(root_key)
    ...
    webbrowser.open("https://profiler.firefox.com/from-url/" + urllib.parse.quote(json_url, safe='') + "?name=" + urllib.parse.quote("Python Profile", safe=''))
...
def get_roots(stats_dict: StatsDict) -> list[FuncKeyTuple]:
    child_to_parent = {}

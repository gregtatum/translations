...
def calc_callers(stats):
    ...
    funcs = {}
    calls = {}
    for func, (calls_count, primitive_calls_count, self_time, ct, clist) in stats.items():
        funcs[func] = {
            "calls": [],
            "called": [],
            "stat": (calls_count, primitive_calls_count, self_time, ct),
        }
        if not clist:
            roots.append(func)
...
def prepare(funcs, calls, threshold=0.0001):
        ...
        if bchilds:
            ...
            bchilds = [
                (
                    ...
                    ff,
                    (
                        round(calls_count * factor),
                        round(primitive_calls_count * factor),
                        self_time * factor,
                        total_time * factor,
                    ),
                    ccnt,
                ...
                for f, ff, (
                    calls_count,
                    primitive_calls_count,
                    self_time,
                    total_time,
                ), ccnt in bchilds
            ]

        for child, _, (calls_count, primitive_calls_count, self_time, total_time), ccnt in (
            gchilds + bchilds
        ):
            if total_time / maxw > threshold:
                ckey = parent, child
                ctrace = trace + (child,)
                block = {
                    "trace": ctrace,
                ...
                if ckey not in visited:
                    _calc(
                        child,
                        (calls_count, primitive_calls_count, self_time, total_time),
                        level + 1,
                        origin,
                        visited | {ckey},
                        ctrace,
    ...
    def _calc_back(names, level, to, origin, visited, pw):
        ...
        for name in sorted(names):
            func = funcs[name]
            if to:
                calls_count, primitive_calls_count, self_time, total_time = calls[(name, to)]
                ttt = total_time * factor
            else:
                calls_count, primitive_calls_count, self_time, total_time = func["stat"]
                ttt = self_time * factor
...
def render_svg(blocks, bblocks, maxw, h=24, fsize=12, width=1200):
    if blocks:
        s1, h1 = render_svg_section(
            blocks, maxw, [COLORS, CCOLORS], h=h, fsize=fsize, width=width, top=0
        )
        h1 += h
    else:
        s1, h1 = [], 0
...
def render(
    ...
    if fmt == "svg":
        print(render_svg(blocks, bblocks, maxw, h=row_height, fsize=fsize, width=width), file=out)
    elif fmt == "log":
...
def get_arg_parser():
    ...
    parser.add_argument(
        "--width", type=int, help="image width, default is %(default)s", default=DEFAULT_WIDTH
    )
    parser.add_argument(
        "--row-height",
        type=int,
    ...
    parser.add_argument(
        ...
        default="{method}.{url}",
        help=(
            "file name template in wsgi mode, default is %(default)s."
            " Possible vars are method, url, ts or duration"
        ),
    )
    parser.add_argument(
        "--pstat", action="store_true", help="Also save profile output into *.pstat file"
    )
    return parser
...
if __name__ == "__main__":
    ...
    if args.run or args.run_module:
        if args.run:
            code = compile(
                open(args.stats, mode="rb").read(), "__main__", "exec", dont_inherit=True
            )
            fname = args.stats
    ...
    try:
        ...
        def pytest_addoption(parser):
            ...
            group.addoption(
                "--flameprof-opts", help="flameprof opts, default out is /tmp/pytest-prof.svg"
            )
        ...
        class PyTestPlugin(object):
            ...
            def pytest_unconfigure(self, config):
                if self.any_test_was_run:
                    ...
                    render(
                        self.profiler.stats,
                        get_out(args.out, "/tmp/pytest-prof.svg"),
                        args.format,
                        args.threshold / 100,
                        args.width,
                        args.row_height,

...
Other flags: `--status` (probe pass only, then exit), `--resume` (explicitly resume the manifest version — the same as a bare run), `--watch-wheels` (after a successful push, poll PyPI until the 5-platform CI wheel matrix completes — see [Binary wheels (CI)](#binary-wheels-ci)), `--no-push` (commit + tag locally, push by hand), `--allow-dirty`, `--skip-tests`, `--remote <name>`.
...
- [ ] Optionally `task rs:publish -- --watch-wheels` (or add it to the release run) to poll the CI wheel matrix to 5/5; a timeout is not a failure.
...
The board reflects this leg as a final **CI wheels** row — a *status-only* row that the local machine never runs (it can't build the matrix). Its probe counts how many of the five platform wheels have landed on PyPI and shows `(N/5)`, going green only at 5/5. Immediately after a release it reads `(0/5) — builds on the tag push`; it fills in as CI finishes each platform. To watch it in one sitting, add `--watch-wheels` (to the release run or on its own afterward): the publisher polls PyPI every 30s for up to 30 min, re-rendering the count until it reaches 5/5. A timeout — or Ctrl-C — is **not** a release failure (the wheels are a CI backfill; the release already succeeded); it just prints the current `(N/5)` and the Actions link, and you can re-dispatch with `gh workflow run pypi-wheels.yml`.

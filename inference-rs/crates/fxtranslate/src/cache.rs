...
//! skips the network.
//!
//! The module splits along the discovery/storage seam. The pure verify+decompress
//! ([`sha256_hex`], [`zstd_decode`], [`verify_and_decompress`]) compile under the
//! `discovery` feature — no `std::fs`, no `dirs` — so they reach the wasm build.
//! The [`Cache`] struct and its `std::fs`/`dirs` storage stay behind `download`,
//! a native-only concern each non-Rust shell reimplements idiomatically.
...
/// Decode a zstd attachment and verify the decompressed bytes against an expected
/// hex SHA-256 (the record's `decompressedHash`). Returns the decompressed bytes
/// on success. This is the pure decision half of a cached download — the crypto
/// and decompression — so a non-Rust shell (JS/Python) does only the raw HTTP and
/// file write and calls this to decode+verify, rather than reimplementing zstd and
/// SHA-256. Pass `expected_sha256_hex` = `None` to decompress without verifying
/// (records that carry no hash are trusted as-is).
...
/// The native `std::fs`/`dirs` cache storage: the [`Cache`] struct, its download +
/// atomic-write path, and enumeration. Behind `download` (not `discovery`) because
/// it needs the filesystem and the platform cache root — a shell concern each
/// non-Rust binding reimplements. Re-exported flat so `cache::Cache` etc. keep
/// their paths.
...
// Flatten the native storage surface back onto `cache::` so `cache::Cache`,
// `cache::ModelFiles`, `cache::ensure_model`, etc. keep the paths every native
// caller already uses.
#[cfg(feature = "download")]
pub use storage::*;
